...
void PCA9685::setChannelAngel(int channel, int Angel) {
    if (channel < 0 || channel > 15) return;

#ifdef PCA9685_ENABLE_DEBUG_OUTPUT
    Serial.println("PCA9685::setChannelAngel");
#endif

    writeChannelBegin(channel);
    uint16_t pwmAmount;
    
    pwmAmount = ( Angel/180 * (PCA9685_SERVO_PULSE_MAX - PCA9685_SERVO_PULSE_MIN) + PCA9685_SERVO_PULSE_MIN) * (4095 /  (1000 / _pwmFrequency));
