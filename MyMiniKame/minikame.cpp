...
void MiniKame::init(){
    ...
    servo_channel[0] = 0; // Servo S0
    servo_channel[1] = 1; // Servo S1
    servo_channel[2] = 2; // Servo S2
    servo_channel[3] = 3; // Servo S3
    servo_channel[4] = 4; // Servo S4
    servo_channel[5] = 5; // Servo S5
    servo_channel[6] = 6; // Servo S6
    servo_channel[7] = 7; // Servo S7
    ...
    // Init PCA9685 servoController
    Wire.begin();                                      // Wire must be started first
    Wire.setClock(400000);                             // Supported baud rates are 100kHz, 400kHz, and 1000kHz
    servoController.resetDevices();                    // Software resets all PCA9685 devices on Wire line
    servoController.init(B000000);                     // Address pins A5-A0 set to B000000
    servoController.setPWMFrequency(SERVO_FREQUENCY);  // Default is 200Hz, supports 24Hz to 1526Hz
...
void MiniKame::setServo(int id, float target){
    if (!reverse[id])
        servoController.setChannelPWM(id, angToPWM(target+trim[id]));
    else
        servoController.setChannelPWM(id, angToPWM(180-(target+trim[id])));
...
int MiniKame::angToPWM(float value){
    return (value/180 * (SERVO_PULSE_MAX - SERVO_PULSE_MIN) + SERVO_PULSE_MIN) * (4095 * SERVO_FREQUENCY) / 1000;
}
